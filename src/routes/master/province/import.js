...
module.exports = async (fastify, options) => {
  ...
  async (request) => {
    ...
    for (let i = 1; i < length; i++) {
      ...
      if (!['ใต้', 'กลาง', 'ตะวันออกเฉียงเหนือ', 'ตะวันออก', 'ตะวันตก', 'เหนือ'].includes(province[1])) return fastify.httpErrors.badRequest(`ภูมิภาคไม่ถูกต้อง '${province[1]}' แถวที่ [${i+1}]`)
